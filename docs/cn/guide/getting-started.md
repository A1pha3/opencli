...
如果你不是通过全局安装使用 OpenCLI，而是在仓库源码里开发或试跑命令，常见的运行方式有三种。

### 方式一：开发模式运行源码

适合日常开发。命令会直接执行 `src/main.ts`，不需要先构建。
...
这一方式最适合你修改了 `src/`、`clis/` 或 YAML 适配器后立即验证效果。
...
如果你想验证“用户安装后实际拿到的程序能不能跑”，优先使用这一方式。

### 方式三：将本地仓库链接为 opencli 命令

适合希望在终端里直接输入 `opencli`，同时继续使用当前仓库源码的开发者。
...
当你修改源码后，如果改动影响到 `dist/`，通常需要重新运行 `npm run build`；如果你走的是 `npm run dev`，则不依赖这一步。
...
1. **启动 Daemon**：第一次运行需要浏览器的命令时，Daemon 会自动启动：
...
1. **确认扩展连接**：当 Daemon 启动后，Chrome 扩展会自动连接。如果扩展图标显示连接状态正常，即可使用。
...
如果你当前是在源码仓库里开发，对应命令可以替换成：

```bash
npm run dev -- --help
npm run dev -- bilibili --help
npm run dev -- bilibili search --help
```
...
```bash
npm start -- --help
npm start -- bilibili --help
npm start -- bilibili search --help
```
...
### 在仓库里应该用 `npm run dev` 还是 `npm start`

- `npm run dev`：直接运行源码，适合开发和调试。
- `npm start`：运行 `dist/` 中的构建产物，适合验证发布结果。
- `opencli ...`：通常用于全局安装后，或者你已经执行过 `npm link`。

如果你刚修改了适配器逻辑，先用 `npm run dev -- <site> <command>`；如果你想确认编译产物是否正确，再用 `npm run build && npm start -- <site> <command>`。
